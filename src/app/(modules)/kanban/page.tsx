...
export default function KanbanPage() {
  ...
  const [contacts, setContacts] = useState<Contact[]>(mockKanbanContacts);
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    // Update cards array
    const newCards = cards.map((c) => (c.id === draggableId ? updatedCard : c));

    // Reorder cards in the destination pipeline
    const pipelineCards = newCards
      .filter((c) => c.pipeline_id === destination.droppableId)
      .sort((a, b) => a.position - b.position);

    pipelineCards.forEach((c, index) => {
      c.position = index;
    });
  ...
  return (
    <div className="bg-background p-6 h-full overflow-hidden">
